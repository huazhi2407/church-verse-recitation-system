...
Next.js + Firebase 建置的背經文工具：註冊／登入（帳號編號＋密碼）、管理員填寫**一週經文**（每天累加）、**錄音 + AI 感測簽到**、**週曆表格**進度追蹤。
...
系統不會自動建立管理員。請在 Firebase 主控台 → Firestore → `users` 集合中，找到要設為管理員的帳號編號文件（例如 `0001`），將欄位 `role` 從 `member` 改為 `admin`。之後該帳號登入即可使用「管理經文」與查看所有人的週曆表格。
